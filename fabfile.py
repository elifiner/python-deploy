...
source_supervisor_conf = '%s/etc/supervisor.conf' % app_dir
system_supervisor_conf = '/etc/supervisor/conf.d/%s.conf' % app_name
source_nginx_conf = '%s/etc/nginx.conf' % app_dir
...
def _configure_supervisor():
    sudo('ln -s %s %s' % (source_supervisor_conf, system_supervisor_conf))
    sudo('supervisorctl reread')
    sudo('supervisorctl update')

def _configure_nginx():
    sudo('ln -s %s %s' % (source_nginx_conf, system_nginx_conf))
